...
Récupérer le fichier contenant les caractéristiques des pokemons 
...
Faites varier ces deux paramètres dans un intervalle de $[50% ; 400%]$ de leur valeur, discrétisé en 50 valeurs.
...
variation_spl_size  <- round(seq(from= 0.5*(length(fire_pokemons$HP)), to= 4*(length(fire_pokemons$HP)), length.out= 50 ))
...
pss <-  ggplot(resultats_SA, aes(x=taille_echantillon, y=moyenne ))+
...
# Bootstrap dans le cas d'une regression linéaire 


Nous allons appliquer le bootstrap à la régression linéaires pour observer la dispersion des coefficients de la droite de régression et calculer des intervalles de confiance sur ces coefficients. 
