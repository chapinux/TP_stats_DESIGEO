...
```{r lilimodif}
lili2[[2]] <- "bla bla bla bla pookie"
lili2
```
...
les matrices sont des tableaux à deux dimensions pour lesquels sont défionies plusieurs fonctions de calcul matriciel.
une matrice se définit par exemple avec la fonction `matrix()`, en renseignant le contenu de la matrice (argument `data`), et la dimension de celle-ci (arguments `nrow` et `ncol`).

Si le nombre de valeurs est inférieur à la taille de la matrice, les valeurs sont **recyclées** , comme le montre l'exemple ci-dessous :

```{r matrix1}
mama <-  matrix(data = 1, nrow = 3, ncol=3)
mama
```
...
les colonnes d'un dataframe sont des **vecteurs**, et ce que nous avons vu à leur sujet s'applique.
...
il faut pour cela qu'ils aient les mêmes colonnes (nombre et type).
...
Pour lire un fichier , il faut être capable d'exprimer son **chemin d'accès** (path en anaglais) dans le format adapté à votre système d'exploitation. 
Reportez-vous à ce document [TODO Lien] pour des précisions.
...
6. A partir de ce jeu de données, on vous demande de créer un second dataframe qui ne contienne que les données des joueurs qui ont perdu les  matchs de la compétition . Dans les données , c'est la colonne `result` qui porte cette information, la valeur `0` signifiant une défaite.   
