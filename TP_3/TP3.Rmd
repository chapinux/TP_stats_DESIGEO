...
R a simplement noté la présence de groupe comme l'iondique la deuxième ligne de la sortie : `Groups:homeworld [49]`, et effectuera des traitements par groupe si on lui demande dans une étape ultérieure. 
...
Toute fonction qui retourne un résultat peut être utilisée pour peupler la nouvelle valeur, comme le montre la seconde variable `classe_taille`, qui n'utilise pas particulièrement le regroupement et donne, pour chaque individu une chaîne de caractère fonction de sa variable `height`.  
